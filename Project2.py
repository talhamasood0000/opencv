...
def preProcessing(img):
    ...
    # dilataion, erosion to make thineer ot thicker
    kernal=np.ones((5,5))
    imgDilation=cv2.dilate(imgCanny,kernal,iterations=2)
    imgThres=cv2.erode(imgDilation,kernal,iterations=1)
    ...
    for cnt in countours:
        ...
        if area>5000:
            ...
            if area>maxArea and len(approx)==4:
                biggest=approx
                maxArea=area
    cv2.drawContours(imgContour,biggest,-1,(255,0,0),3) #highlight the corner points of the document
    return biggest
...
def getWrap(img,biggest):
    ...
    pts1=np.float32(biggest) #add points
...
while True:
    ...
    if biggest.size !=0:
        ...
        imagArray=([img,imgThres],[imgContour,imgWrapped])
    else:
        imagArray=([img,imgThres],[img,img])


    stackIM=stackImages(0.6,imagArray)

    cv2.imshow('Video',stackIM)
